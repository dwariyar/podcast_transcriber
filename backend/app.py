...
# RSS_URL placeholder for initial workflow instance creation
...
async def transcribe_podcast():
    logging.info("Received request to /transcribe endpoint.")
    ...
    if not openai_api_key or not algolia_app_id or not algolia_write_api_key:
        logging.warning("Missing one or more API keys in request payload.")
        return jsonify({"error": "Missing one or more API keys in request payload."}), 400

    logging.info(f"Transcription request for RSS URL: {rss_url_from_frontend}, Episodes: {num_episodes}, Sample Duration: {sample_duration}s")
    ...
    try:
        ...
        logging.error(f"Workflow configuration error: {e}")
